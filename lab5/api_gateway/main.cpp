...
public:
    ...
    HTTPRequestHandler *createRequestHandler(const HTTPServerRequest &request) {
        cout << "get request: " << request.getURI() << endl;
        if (request.getURI().find("/route") != -1 || request.getURI().find("/travel") != -1 || request.getURI().find("/user") != -1) {
            cout << "start\n";
...
protected:
    ...
    {
            ServerSocket ss(8082); // does bind + listen
