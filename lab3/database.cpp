...
DatabaseMongo::DatabaseMongo(): database_mongo("archdb"/*getenv("MONGO_DATABASE") */) {

    string mongo = "127.0.0.1";
...
std::vector<string> DatabaseMongo::GetFromMongo(const string& collection, Poco::MongoDB::Document &docr) {
    ...
    try { 
        ...
        cout <<"aaaaaaaa\n";
        cout << docr.toString() << endl;

        connection_mongo.sendRequest(*request, response);

        cout << "res size = " << response.documents().size() << endl;
